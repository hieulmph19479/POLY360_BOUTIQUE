...
public interface ThongKeRepository extends JpaRepository<HoaDon, Long> {
    ...
    @Query("SELECT COALESCE(SUM(hd.tongTien), 0) FROM HoaDon hd WHERE DAY(hd.ngayTao) = :day AND MONTH(hd.ngayTao) = :month AND YEAR(hd.ngayTao) = :year AND hd.trangThai = 1")
    Long revenueDayMonthYear(@Param("day") int day, @Param("month") int month, @Param("year") int year);
